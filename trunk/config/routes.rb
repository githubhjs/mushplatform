ActionController::Routing::Routes.draw do |map|
  ...
  map.connect   "/user/forget_password",:controller => 'user',:action => 'forgot_password'
#  map.connect   "/manage",:controller => 'manage/blogs',:action => 'index'
  ...
  map.with_options :controller => "my_space",:conditions => { :subdomain => /^(?!www$)\w+/ } do  |my_space|
    ...
    my_space.connect '/messages/page/:page',:action => 'messages'
    my_space.connect '/messages',:action => 'messages'
    my_space.connect '/friends/page/:page',:action => 'friends'
    my_space.connect '/friends',:action => 'friends'
    my_space.connect '/votes/page/:page',:action => 'votes'
    my_space.connect '/votes',:action => 'votes'
    my_space.connect '/votes/:id',:action => 'vote'
    my_space.connect '/groups/page/:page',:action => 'groups'
  ...
  map.namespace :manage do |manage|
    ...
    manage.resources :friends, :collection => {:search => :any,:visitores => :get,:receiv_invites => :get,:send_invites => :get,:footsteps => :get},
