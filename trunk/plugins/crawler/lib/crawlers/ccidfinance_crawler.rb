...
#    @latest_artice = CrawlerArticle.find(:first,:order => "created_at_site desc",:select => "created_at_site")
  ...
  def parse_latest_update_pages(update_page_path)
    ...
    unless doc.nil?
      doc.search("//td[@width='70%']/a").each{|a|
...
puts "begin to fetch"
site = CrawlerSite.find_by_site_name('ccidfinance')
if site
  crawler = CcidfinanceCrawler.new(site)
  crawler.fetch
