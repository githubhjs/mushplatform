class MenuBarsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @menu_bar.save
        flash[:notice] = 'MenuBar was successfully created.'
        format.html { redirect_to(@menu_bar) }
  ...
  def update
    ...
    respond_to do |format|
      if @menu_bar.update_attributes(params[:menu_bar])
        flash[:notice] = 'MenuBar was successfully updated.'
        format.html { redirect_to(@menu_bar) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(menu_bars_url) }
      format.xml  { head :ok }
    end
  end
end
